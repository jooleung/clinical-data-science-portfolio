...
export function ProjectCard({ project }: { project: Project }) {
  return (
    <article className="pixel-panel overflow-hidden transition-transform hover:-translate-y-1">
      <div className="relative aspect-[16/9] border-b-2 border-ink bg-panel-100">
      ...
      <div className="p-5">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {project.tags.map((tag) => (
            <span key={tag} className="border border-ink/35 bg-white/50 px-2.5 py-1 font-mono text-[0.68rem] font-semibold uppercase tracking-[0.08em] text-ink/75">
              {tag}
            </span>
          ))}
        </div>
